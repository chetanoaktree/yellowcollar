...
const com = ({placeholder='status, Influencer name..', extract=[], status_options, updateHandler, ...props}) => {   
 
  const[isFilter, setFilter] = useState({query:'', status:'', business_id:'', business_name:'', influencer_id:'', influencer_name:'', product_id:'', product_name:'', start:1, end:10, limit:10})  
  ...
  const resetHandler = () =>{
    setFilter(prev=>{
      let next={...prev, query:'', status:'', influencer_id:'', influencer_name:'', product_id:'', product_name:''}
      updateHandler(next)     
      return next
    })    
  }
