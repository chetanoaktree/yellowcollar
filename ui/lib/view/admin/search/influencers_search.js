...
const com = ({status_options, updateHandler, ...props}) => {   
 
  const[isFilter, setFilter] = useState({query:'', status:'', business_id:'', business_name:'', influencer_id:'', influencer_name:'', product_id:'', product_name:''})  
  ...
  const applyHandler = () =>{
    let out=extract_items(isFilter, [
      {find:'status', get:'status', get_type:'object'},     
      ...
      {find:'s', get:'status', get_type:'object'},
  ...
  const resetHandler = () =>{
    setFilter({query:'', status:'', influencer_id:'', influencer_name:'', product_id:'', product_name:''})
    updateHandler({})     
  }
    ...
    <div className={s.search}>
      <div className={s.content+' flex -mx-1'}>
        ...
        <div className={s.input_g+' w-4/12 px-1'}>
          ...
          <Select name="status" value={isFilter.status} options={status_options} placeholder="status" changeHandler={selectHandler}/>
        </div>          
      </div>      
    </div>
  ) 
  ...
  return (
    <div className={s.search_area}>
      <Search value={isFilter.query} placeholder="status, Influencer name" queryHandler={queryHandler} applyHandler={applyHandler} resetHandler={resetHandler}>
        {SearchContent}
      </Search> 
    </div>   
  )
