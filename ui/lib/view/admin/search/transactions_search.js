...
const com = ({collab_status_options, updateHandler, ...props}) => {   
  ...
  const resetHandler = () =>{
    setFilter({query:'', collab_status:'', business_id:'', business_name:'', influencer_id:'', influencer_name:'', product_id:'', product_name:''})
    updateHandler({})     
  }
  ...
  return (
    <div className={s.search_area}>
      <Search value={isFilter.query} placeholder="Search Buesiness, Influencer, Product, status" queryHandler={queryHandler} applyHandler={applyHandler} resetHandler={resetHandler}>
        {SearchContent}
      </Search> 
    </div>   
  )
