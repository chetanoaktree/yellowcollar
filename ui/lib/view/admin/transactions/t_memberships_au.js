...
const com = (props) => { 
  ...
  const Item=(i)=>{
    ...
    let {id, created_at, user_type, order_id, new_changes={}, influencer, business, meta={}} =isL    
    let {final_price, final_save, price, plan, duration} = meta ? meta : {} 
  ...
  //  /console.log("isA --", isA)
    ...
    let from_user = user_type=='influencer' ?  influencer.name : (business.company_name ?  business.company_name : business.name) 
    let duration_ = duration > 1 ? duration+' months' : duration+' month'
    let content_=(
      <div className={"flex items-stretched "}>        
        <div className={s.product+" w-4/12 flex mr-2"}>          
          <div className={"w-full"}>
            <div className={s.gray+" rounded-xl mb-2"}>
              <div className={"text-lg w-full"}>{from_user}</div> 
              <div className={"opacity-50 text-sm mb-2"}>                
                <div className={'mr-2 w-full '}>{user_type} </div>                              
              </div>                          
            </div>            
        ...
        <div className={s.gray+" w-4/12 rounded-xl text-xs p-4 mr-2"}>
          <Details {...{
            ...
            items:[
              ...
              {label:'Duration', value:duration_},             
            ]
          }}/>  
        </div>
        <div className={s.gray+" w-4/12  rounded-xl text-xs p-4 "}> 
          <Details {...{
            ...
            items:[                         
              {label:'Price', value:amount({v:price})},
              {label:'Discount', value:amount({v:final_save})},
              {label:'Final Price', value:amount({v:final_price})},  
  ...
  const render = ()=>{
    ...
    let DG_args={    
       ...
       items,
       header:{
         title:(<h4>Membership Transactions</h4>), 
