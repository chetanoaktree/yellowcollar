...
const com = (props) => { 
  ...
  const Item=(i)=>{
    ...
    let {id, created_at, collab_id,  new_changes={}, collab, meta={}} =isL    
    let {test, sub_total, platform_fee} = meta ? meta : {}   
    let {business, product, influencer} = collab ? collab : {}     
    let amount_ = (meta && meta.amount) ? meta.amount : 0
        ...
        <div className={s.product+" w-4/12 flex mr-2"}>          
          <div className={"w-full"}>
            <div className={s.gray+" rounded-xl mb-2"}>
              <div className={"text-lg w-full"}>{business.company_name}</div>                           
            </div>            
            <div className={"text-xl"}>{collab_status_tag({...isL, size:'sm'})}</div>
          </div>
        </div>
        <div className={s.gray+" w-4/12 rounded-xl text-sm p-4 mr-2"}>
          <Details {...{
            header:{
              title:'Collab Details'
            },
            items:[
              {label:'ID', value:'#'+collab_id},
              {label:'Influencer', value:influencer.name},
              {label:'Product', value:product.title},
            ]
          }}/>  
        </div>
        <div className={s.gray+" w-4/12  rounded-xl text-sm p-4 "}> 
          <Details {...{
            header:{
            ...
            items:[
              ...
              {label:'Platform fee', value:amount({v:platform_fee})},
              {label:'Total', value:amount({v:amount_})},
  ...
  const render = ()=>{
    ...
    let DG_args={    
       ...
       items,
       header:{
         title:(<h4>Collab Transactions</h4>), 
